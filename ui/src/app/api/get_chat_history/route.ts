  ...
  try {
    ...
    if (restApiEndpoint) {
      try {
        ...
        const upstreamHistory = Array.isArray(rawApiData?.history)
          ? rawApiData.history
          : Array.isArray(rawApiData?.data?.history)
            ? rawApiData.data.history
            : [];

        // Convert timestamp from Python datetime string to number for consistency
        const processedData: ChatHistoryResponse = {
          history: upstreamHistory.map((item: any) => ({
            ...item,
            time:
