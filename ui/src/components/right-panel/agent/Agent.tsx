...
export default function Agent({
  ...
  }, [effectiveTraceId, isInitialized]);
  ...
  const handleNewChat = () => {
    ...
    if (existingNewChatTab) {
      ...
      setActiveChatId(null);
      setActiveTempId(existingNewChatTab.tempId);
    } else {
  ...
  const handleChatSelect = async (chatId: string | null, tempId?: string) => {
    ...
    setActiveTempId(chatId ? undefined : tempId);
  };
  ...
  const handleChatClose = (chatId: string | null, tempId?: string) => {
    setChatTabs((prev) => {
      ...
      const isClosingActiveChat =
        ...
        (chatId === null && activeChatId === null && tempId === activeTempId);

      if (isClosingActiveChat && filtered.length > 0) {
        setActiveChatId(filtered[0].chatId);
        setActiveTempId(filtered[0].chatId ? undefined : filtered[0].tempId);
      }
  ...
  const updateChatMessages = (
    ...
    setChatTabs((prev) =>
      prev.map((tab) =>
        (chatId !== null
          ? tab.chatId === chatId
          : tab.chatId === null && tab.tempId === tempId)
  ...
  const handleHistoryItemsSelect = async (chatIds: string[]) => {
    for (const chatId of chatIds) {
      ...
      try {
        ...
        updateChatMessages(chatId, undefined, historyMessages);
      } catch (error) {
  ...
  const sendMessage = async (message: string) => {
    ...
    updateChatMessages(currentChatId, currentTempId, [...messages, userMessage]);
    ...
    const fetchSpecialMessages = async () => {
      try {
        const token = await getToken();
        const historyResponse = await globalThis.fetch(
    ...
    try {
      ...
      if (chatResponse.success && chatResponse.data) {
        ...
        const assistantMessage: Message = {
          ...
          action_type: chatResponse.data!.action_type,
          status: chatResponse.data!.status,
          metadata: chatResponse.data!.metadata,
          user_feedback: chatResponse.data!.user_feedback,
        };
